...
public class GameLogic {
	...
	public GameLogic(){ // constructor for game logic class
...
//    }
	...
	public boolean winCondition(String color, Board board){ // method for checking if win condition has been meet; return true if game is won, return false if not; must pass in color for player that is being checked
		...
		if (numPieces(color, board)) { // checking if number of pieces in less than or equal to 2
			...
			if (noAvailableMoves(color, board.getPieces("white"), board.getPieces("black"), board.getSpaces())) {
				System.out.print(color + " has lost!");
				return true;
			}
		}
			...
			System.out.println("Calling availableMoves " + color);
			if (noAvailableMoves(color, board.getPieces("black"), board.getPieces("white"), board.getSpaces())) {
				System.out.print(color + " has lost!");
				return true;
			}
		}
	...
	// called within winCondition, verifying a player has no valid moves remaining 
	public boolean noAvailableMoves(String color, List<Integer> playerPieces, List<Integer> opponentPieces, HashMap<Integer, List<Integer>> spaces) { // checks to make sure player provided has available moves, if not game lost
